...
const PropertyDetailsModal = lazy(() =>
  import("./components/PropertyDetailsModal").then((m) => ({
    default: m.PropertyDetailsModal,
  }))
);
...
const PublicPropertyPage = lazy(() =>
  import("./components/PublicPropertyPage").then((m) => ({
    default: m.PublicPropertyPage,
  }))
);
...
function App() {
  ...
  useEffect(() => {
    ...
    console.log = (...args) => {
      (window as any).appLogs.push(args);
      originalLog(...args);
    };
    return () => {
  ...
  const [activeFilter] = useState<FilterType>(() => loadPersistedFilter());
      ...
      try {
        ...
        const errorMessage =
          error instanceof Error
            ? error.message
            : "Failed to load public properties";
        setToast({ message: errorMessage, type: "error" });
      }
    },
    [pagination, viewMode]
  ...
  useEffect(() => {
    ...
    if (
      ...
      initialLoadOwnerIdRef.current === 0
    ) {
      return;
    }
    ...
    if (hasActiveFilters || hasActiveSearch) {
      ...
      if (hasActiveSearch) {
        requestKey = `search: 0:${listParam}:${searchQuery.trim()}:${searchColumn}:${JSON.stringify(
          sortedFilters
        )
          }: 1: 40`;
      } else {
        requestKey = `filter: 0:${listParam}:${JSON.stringify(
          sortedFilters
        )
          }:${searchQuery.trim()}:${searchColumn}: 1: 40`;
      }
  ...
  }, [location.pathname]); // Only run when authenticated/ownerId changes
  ...
  const applyClientSideFilters = useCallback(
    (properties: Property[], filters: FilterOptions): Property[] => {
      return properties.filter((property) => {
        ...
        if (
          ...
          property.price_min < filters.min_price
        )
          return false;
        if (
          filters.max_price !== undefined &&
          property.price_max > filters.max_price
        )
          return false;
        if (
          filters.size_min !== undefined &&
          property.size_min < filters.size_min
        )
          return false;
        if (
          filters.max_size !== undefined &&
          property.size_max > filters.max_size
        )
          return false;
        if (filters.size_unit && property.size_unit !== filters.size_unit)
          return false;
        if (
          filters.description &&
  ...
  const handleShare = async (property: Property) => {
    ...
    const priceText = formatPriceWithLabel(
      property.price_min,
      property.price_max
    );
    const shareUrl =
  ...
  const handleSearch = useCallback(
    async (query: string, column?: string, filterOverride?: FilterType) => {
      ...
      const requestKey = `search: 0:${listParam}:${query.trim()}:${currentColumn}:${JSON.stringify(
        sortedFilters
      )
      ...
      try {
        ...
        if (query.trim()) {
          ...
          // This avoids loading all properties first, then filtering
          const filterResponse = await propertyApi.filterProperties(
            0,
            listParam,
            activeFilters,
            { page: normalizedPage, per_page: normalizedPerPage },
        ...
        if (
          filterRequestIdRef.current === currentFilterRequestId &&
          pendingFilterRequestRef.current === requestKey
        ) {
          pendingFilterRequestRef.current = null;
  ...
  const handleFilter = useCallback(
    async (filters: FilterOptions, filterOverride?: FilterType) => {
      ...
      const requestKey = `filter: 0:${listParam}:${JSON.stringify(
        sortedFilters
      )
        }:${searchQuery.trim()}:${searchColumn}:${normalizedPage}:${normalizedPerPage} `;
  ...
  useEffect(() => {
    ...
    if (loadedDataRef.current && loadedDataRef.current.ownerId !== 0) {
    ...
    if (hasActiveFilters || hasActiveSearch) {
      // Only trigger filter if we're not already loading and the request is different
      // This prevents infinite loops
      const listParam = activeFilter === "my" ? "mine" : activeFilter === "public" ? "others" : "both";
      const sortedFilters = Object.keys(activeFilters).sort().reduce((acc, key) => {
        (acc as any)[key] = activeFilters[key as keyof FilterOptions];
        return acc;
      }, {} as FilterOptions);

      const requestKey = hasActiveSearch
        ? `search: 0:${listParam}:${searchQuery.trim()}:${searchColumn}:${JSON.stringify(sortedFilters)}:1:40 `
        : `filter: 0:${listParam}:${JSON.stringify(sortedFilters)}:${searchQuery.trim()}:${searchColumn}:1:40 `;

      if (pendingFilterRequestRef.current !== requestKey && !filterLoadingRef.current && !loadingRef.current) {
        handleFilter(activeFilters);
      }
      return;
    }
    ...
    const hasMy =
      loadedDataRef.current?.ownerId === 0 && loadedDataRef.current.my;
    const hasPublic =
      loadedDataRef.current?.ownerId === 0 &&
      loadedDataRef.current.public;
    ...
    if (
      activeFilter === "all" &&
      (!hasMy || !hasPublic || shouldForceReload || paginationChanged)
    ) {
      loadPromises.push(
        (async () => {
          try {
            ...
            console.log('getAllProperties response:', response.data.length);
            // Check again after async operation
            if (requestIdRef.current !== currentRequestId) {
              console.log('Request cancelled after API call');
              return; // Request cancelled
            }
            const allProps = response.data;
            // Split results: my properties have owner_id === ownerId, public properties have owner_id !== ownerId
            const myProps = allProps.filter((p) => p.owner_id === 0);
            const publicProps = allProps.filter((p) => p.owner_id !== 0);
      ...
      if (
        needsPublic &&
        (!hasPublic || shouldForceReload || paginationChanged)
      ) {
        loadPromises.push(
          (async () => {
            try {
  ...
  // Update filteredProperties when base properties change and no filters are active
  // Update filteredProperties when base properties change and no filters are active
  useEffect(() => {
    console.log('Sync Effect running:', {
      searchQuery,
      activeFiltersKeys: Object.keys(activeFilters),
      myProps: myProperties.length,
      publicProps: publicProperties.length
    });
    if (!searchQuery.trim() && Object.keys(activeFilters).length === 0) {
      console.log('Sync Effect: Setting filteredProperties');
      if (activeFilter === "all") {
        setFilteredProperties([...myProperties, ...publicProperties]);
      } else if (activeFilter === "my") {
        setFilteredProperties(myProperties);
      ...
      console.log('Sync Effect: Skipping update due to active filters/search');
    }
  }, [myProperties, publicProperties, activeFilter, searchQuery, activeFilters]);
  ...
  const handleClearSearchAndFilters = useCallback(() => {
    ...
    // Force reload of base properties since we might have skipped loading them initially
...
function MainAppContent({
  ...
  const applyClientSideFilters = useCallback(
    (properties: Property[], filters: FilterOptions): Property[] => {
      return properties.filter((property) => {
        ...
        if (
          filters.tags &&
          !property.tags?.toLowerCase().includes((Array.isArray(filters.tags) ? filters.tags.join(' ') : filters.tags).toLowerCase())
        )
          return false;
        if (
          filters.highlights &&
          !property.highlights
            ?.toLowerCase()
            .includes((Array.isArray(filters.highlights) ? filters.highlights.join(' ') : filters.highlights).toLowerCase())
        )
          return false;
        return true;
      });
  ...
  const propertiesForMap = sourceProperties.filter((property) => {
    ...
    const hasLocation =
      property.location &&
      /^-?\d+\.?\d*,-?\d+\.?\d*$/.test(property.location.trim());
    // Check if property has landmark location
    const hasLandmark =
      property.landmark_location &&
      /^-?\d+\.?\d*,-?\d+\.?\d*$/.test(property.landmark_location.trim());
  ...
  const shouldShowPagination =
    paginationMeta !== null || !hasActiveSearchOrFilter;
  ...
  const getMapCenter = (): [number, number] => {
    ...
    propertiesWithCoords.forEach((property) => {
      const coords = property.location
        .split(",")
