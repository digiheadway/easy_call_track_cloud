...
$searchEngineId = 'f6f88d03446354838';
// Getting Ready Api Request
$query = urlencode($searchquery);
$types = array("movie", "movies", "web series", "webseries", "web");
$containsType = !empty(array_filter($types, fn($type) => stripos($query, $type) !== false));

$query_suffix = $containsType ? "poster+hd" : "Movie+poster+hd";
$final_query = $query . "+" . $query_suffix;

while ($row = $result->fetch_assoc()) {
    $apiKey = $row["api_key"];

    // Make API request
    $url = "https://www.googleapis.com/customsearch/v1?key=$apiKey&cx=$searchEngineId&q=$final_query&searchType=image";
    $response = file_get_contents($url);
    $responseData = json_decode($response, true);
    setcookie('ad_gauid', 'gfgfbAA' . $apiKey . 'BBdfvdfv', time() + 300); // Cache for 5 min

    if (isset($responseData['spelling']) && isset($responseData['spelling']['correctedQuery'])) {
        // Corrected query exists
        $imageUrl = isset($responseData['items'][0]['link']) ? $responseData['items'][0]['link'] : '/assets/img/not-found.jpg';
        $correctedQuery = $responseData['spelling']['correctedQuery'];

        $query = str_replace('+', ' ', $query);
        $query_suffix = str_replace('+', ' ', $query_suffix);
        ...
        $conn->query("INSERT INTO queries (query, correct, imageUrl) VALUES ('$query', '$correctedQuery', '$imageUrl') ON DUPLICATE KEY UPDATE correct = '$correctedQuery'");
        echo json_encode(array('corrected_query' => $correctedQuery));
        break; // Exit the loop if result is found
    } elseif (isset($responseData['items'][0]['link'])) {
        // If result found
