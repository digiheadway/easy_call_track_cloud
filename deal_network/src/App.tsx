...
function App() {
  ...
  // Use property handlers hook for all CRUD operations
  const {
    handleAddProperty: addPropertyBase,
    handleUpdateProperty: updatePropertyBase,
    handleDeleteProperty: deletePropertyBase,
    handleTogglePublic: togglePublicBase,
    handleUpdateHighlightsAndTags: updateHighlightsAndTagsBase,
    handleUpdateLocation: updateLocationBase,
    handleUpdateLandmarkLocation: updateLandmarkLocationBase,
    handleFavProperty: favPropertyBase,
  } = usePropertyHandlers({
    ownerId,
    onSuccess: (message) => showToast(message, 'success'),
    onError: (message) => showToast(message, 'error'),
    onPropertyUpdated: () => refreshPropertiesAndFilters(true),
  });

  // Wrapper for handleAddProperty with navigation logic
  const handleAddProperty = async (data: PropertyFormData) => {
    const newPropertyId = await addPropertyBase(data);
  ...
  // Wrapper for handleEditProperty with modal logic
  const handleEditProperty = async (data: PropertyFormData) => {
    if (!editingProperty) return;
    const success = await updatePropertyBase(editingProperty.id, data);
    if (success) {
      setShowModal(false);
  ...
  // Wrapper for handleDeleteProperty with modal logic
  const handleDeleteProperty = async (id: number) => {
    const success = await deletePropertyBase(id);
    if (success) {
      setShowDetailsModal(false);
  ...
  // Direct use of base handlers for simple cases
  const handleTogglePublic = togglePublicBase;
  const handleUpdateHighlightsAndTags = updateHighlightsAndTagsBase;
  const handleUpdateLocation = updateLocationBase;
  const handleUpdateLandmarkLocation = updateLandmarkLocationBase;

  // Wrapper for handleFavProperty with optimistic updates
  const handleFavProperty = async (id: number, isFavourite: boolean, userNote: string) => {
    ...
    // Optimistically update property lists
    ...
    // Call base handler
    const success = await favPropertyBase(id, isFavourite, userNote);
