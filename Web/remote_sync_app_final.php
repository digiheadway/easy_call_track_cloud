...
if ($action === "verify_pairing_code") {
    ...
    if ($employee['device_id'] === null || $employee['device_id'] === '') {
        ...
        if ($employee['device_id'] === $device_id) {
            ...
            // Check if device is linked to another employee
            ...
            if ($stmt3->get_result()->num_rows > 0) {
                errorOut("Device linked to another account", 400);
            }
...
if ($action === "start_call") {
    ...
    // Verify device
    $sv = $conn->prepare("SELECT device_id FROM employees WHERE id = ? AND org_id = ?");
    $sv->bind_param("is", $employee_id, $org_id);
    $sv->execute();
    $ev = $sv->get_result()->fetch_assoc();
    if (!$ev || $ev['device_id'] !== $device_id) errorOut("Unauthorized device", 401);

    $stmt = $conn->prepare("INSERT INTO calls (unique_id, org_id, employee_id, device_phone, caller_name, caller_phone, duration, type, upload_status, call_time) 
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?) 
