    ...
    return (
        <div className="flex flex-wrap items-center gap-2">
            {value === 'custom' ? (
                ...
                <div className="flex items-center gap-2 bg-white  p-1.5 rounded-lg border border-gray-200  animate-in fade-in slide-in-from-left-2 duration-200">
                    <span className="text-xs font-semibold text-blue-600  px-2 uppercase tracking-wider">Custom Range</span>
                    <div className="w-px h-4 bg-gray-200 "></div>
                    {onCustomRangeChange && (
                        <>
                            <input
                                type="date"
                                className="text-sm bg-transparent border-none outline-none text-gray-700  px-2 py-1 font-medium hover:bg-gray-50  rounded cursor-pointer"
                ...
                <div className={`flex bg-white  transition-all rounded-lg p-1 gap-1 overflow-x-auto border ${value !== '7days' && value !== 'all_time' ? 'border-blue-500 ring-4 ring-blue-500/5' : 'border-gray-200 '}`}>
...
function DateFilterSettingsModal({ enabledOptions, onSave, onClose }) {
    ...
    return createPortal(
        ...
        >
            <div
                className="bg-white  rounded-xl shadow-2xl w-full max-w-sm overflow-hidden animate-in fade-in zoom-in-95 duration-150 border "
                onClick={(e) => e.stopPropagation()}
            >
                {/* Header */}
                <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100  bg-gray-50 ">
                    <div>
                        <h3 className="font-semibold text-gray-900 ">Customize Date Filters</h3>
                        <p className="text-xs text-gray-500  mt-0.5">Settings apply to all screens</p>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-4 space-y-2 max-h-[400px] overflow-y-auto">
                    {ALL_DATE_OPTIONS.map((opt) => {
                        ...
                        return (
                            ...
                            >
                                <div className={`w-5 h-5 rounded-md flex items-center justify-center flex-shrink-0 ${isEnabled ? 'bg-blue-600' : 'border-2 border-gray-300 '
                                    }`}>
