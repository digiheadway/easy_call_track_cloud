...
switch ($dateRange) {
    case 'today':
        $dateFilter = "AND DATE(call_time) = CURDATE()";
        break;
    case 'week':
        $dateFilter = "AND call_time >= DATE_SUB(NOW(), INTERVAL 7 DAY)";
        break;
    case 'month':
        $dateFilter = "AND call_time >= DATE_SUB(NOW(), INTERVAL 30 DAY)";
        break;
    case 'quarter':
        $dateFilter = "AND call_time >= DATE_SUB(NOW(), INTERVAL 90 DAY)";
        break;
    case 'year':
        $dateFilter = "AND call_time >= DATE_SUB(NOW(), INTERVAL 365 DAY)";
        break;
    default:
        $dateFilter = '';
}
    ...
    case 'overview':
        ...
        $metrics = Database::getOne("
            SELECT 
                ...
                SUM(CASE WHEN type = 'Incoming' THEN 1 ELSE 0 END) as inbound_calls,
                SUM(CASE WHEN type = 'Outgoing' THEN 1 ELSE 0 END) as outbound_calls,
                SUM(CASE WHEN type != 'Missed' AND duration > 0 THEN 1 ELSE 0 END) as completed_calls,
                SUM(CASE WHEN type = 'Missed' OR duration = 0 THEN 1 ELSE 0 END) as missed_calls,
                AVG(CASE WHEN type != 'Missed' AND duration > 0 THEN duration ELSE NULL END) as avg_duration,
    ...
    case 'employee':
        $employees = Database::select("
            SELECT 
                ...
                SUM(CASE WHEN c.type != 'Missed' AND c.duration > 0 THEN 1 ELSE 0 END) as completed_calls,
                AVG(CASE WHEN c.type != 'Missed' AND c.duration > 0 THEN c.duration ELSE NULL END) as avg_duration,
        ...
        foreach ($employees as &$employee) {
            ...
            $completionScore = $employee['total_calls'] > 0 
                ...
                : 0;
            $recordingScore = min(25, ($employee['recordings_count'] / max(1, $employee['total_calls'])) * 25);
